...
export class StripeService {
  ...
  constructor() {
    this.stripePromise = loadStripe(environment.stripPublicKey);
  }
  ...
  async confirmPayment(confirmPaymentToken: ConfirmationToken) {
    const stripe = await this.getStripeInstance();
    const elements = await this.initializeElements();
    const result = await elements.submit();
    if (result.error) throw new Error(result.error.message);
    ...
    if (stripe && clientSecret) {
      return await stripe.confirmPayment({
        ...
        confirmParams: {
          confirmation_token: confirmPaymentToken.id
        },
  ...
  async createAddressElement() {
    if (!this.elements) {
      ...
      if (elements) {
        ...
        throw new Error('Elements instance is not loaded');
      }
    }
